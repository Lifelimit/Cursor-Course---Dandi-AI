-- Once account deletion begins, prevent creation or reactivation of API keys.
-- This keeps a retryable cross-provider deletion from resetting usable quota.
...
BEGIN
  ...
  SELECT p.plan, p.account_deletion_requested_at
  INTO owner_plan, deletion_requested_at
  ...
  IF deletion_requested_at IS NOT NULL
     AND (TG_OP = 'INSERT' OR (NEW.is_active = true AND (TG_OP = 'INSERT' OR OLD.is_active IS DISTINCT FROM NEW.is_active))) THEN
    RAISE EXCEPTION 'Account deletion is pending; API keys cannot be created or enabled.';
  END IF;

  IF TG_OP = 'INSERT' THEN
    SELECT count(*) INTO total_key_count
  ...
  IF TG_OP = 'INSERT' OR OLD.monthly_limit IS DISTINCT FROM NEW.monthly_limit THEN
    ...
    IF owner_plan <> 'Researcher' THEN
      plan_limit := CASE owner_plan WHEN 'Premium' THEN 5000 ELSE 1000 END;
  ...
  IF (TG_OP = 'INSERT' AND NEW.is_active = true)
    ...
    IF owner_plan <> 'Researcher' THEN
      key_limit := CASE owner_plan WHEN 'Premium' THEN 10 ELSE 3 END;
...
CREATE OR REPLACE FUNCTION public.begin_account_deletion(p_profile_id text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
