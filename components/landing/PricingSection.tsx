...
export function PricingSection({ 
  ...
  return (
    <section id="pricing" className="bg-zinc-950/40 py-12 md:py-40 backdrop-blur-sm border-y border-white/5">
      <div className="mx-auto max-w-7xl px-6">
        <div className="mb-20 text-center space-y-8">
          ...
          <div className="flex items-start justify-center gap-4">
            ...
            <div className="flex flex-col items-center gap-1.5">
              <button 
                ...
                className="relative h-6 w-12 rounded-full bg-zinc-800 border border-white/5 p-1 transition-colors hover:bg-zinc-700 cursor-pointer"
              >
                <div className={`h-4 w-4 rounded-full bg-zinc-100 shadow-sm transition-transform ${billingInterval === "year" ? "translate-x-6" : "translate-x-0"}`} />
              </button>
              <span className="invisible text-[8px] px-1.5 py-0.5 select-none">20% OFF</span>
            </div>
            <div className="flex flex-col items-center gap-1.5">
          ...
          <div className="mx-auto max-w-xl mt-12 p-4 sm:p-6 md:p-8 rounded-[32px] border border-white/5 bg-zinc-950/40 backdrop-blur-sm shadow-none space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-1 text-left">
                <p className="text-[9px] font-black uppercase tracking-[0.25em] text-zinc-500">Volume Estimator</p>
                <h4 className="text-base font-bold text-white">{activeSliderStep.labelDetails} / mo</h4>
                <p className="text-[11px] text-zinc-400">Fits perfectly in the <span className="font-bold text-emerald-400">{activeSliderStep.plan}</span> plan</p>
              </div>
              <div className="rounded-2xl bg-zinc-900/40 px-4 py-2.5 border border-white/5 text-left font-sans">
                <p className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest">Key Limits</p>
                <p className="text-sm font-bold text-zinc-200 mt-0.5">{activeSliderStep.keys}</p>
                <p className="text-[8px] text-zinc-500 mt-0.5">included in selection</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <input 
                ...
                className="w-full h-1.5 rounded-lg bg-zinc-800 accent-emerald-400 cursor-pointer"
              />
              <div className="flex justify-between text-[8px] sm:text-[9px] font-bold uppercase tracking-wider sm:tracking-widest text-zinc-550 px-1">
                {SLIDER_STEPS.map((step, idx) => (
                  <button 
                    key={idx}
                    onClick={() => setSliderIndex(idx)}
                    className={`transition-colors ${sliderIndex === idx ? "text-white" : "hover:text-zinc-450"}`}
                  >
                    {step.label}
                  </button>
                ))}
            ...
            <div className="rounded-2xl bg-zinc-950 p-4 border border-white/5 text-left flex gap-3 items-center">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-zinc-100 text-zinc-950">
                <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor">
                  <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <div>
                <p className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest">Capacity Guidance</p>
                <p className="text-xs font-semibold text-zinc-300 mt-0.5">{activeSliderStep.info}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-3">
          {PLANS.map((plan) => {
            ...
            const usageRecommendationClass = isRecommendedByVolume && !isCurrent
              ? "ring-2 ring-emerald-400 ring-offset-4 ring-offset-zinc-950 scale-[1.03] z-10 shadow-2xl"
              : "";
            
            const baseContainerClass = `border-white/5 bg-zinc-950/40 text-white ${usageRecommendationClass}`;
            const containerClass = isCurrent
              ? "border-2 border-emerald-500 bg-[#0b1020]/95 text-white shadow-2xl shadow-emerald-500/10 ring-1 ring-emerald-500"
              : baseContainerClass;
            ...
            return (
              ...
              >
                ...
                {activeSession ? (
                  <button
                    ...
                    className={`w-full rounded-full py-4 text-[10px] font-black uppercase tracking-widest transition-all ${
                      isCurrent 
                        ...
                        : "bg-zinc-100 text-zinc-950 hover:bg-zinc-200 shadow-xl"
                    }`}
                  >
                    {isLoading ? (
                  ...
                  <Link
                    href="/signup"
                    className="w-full rounded-full py-4 text-center text-[10px] font-black uppercase tracking-widest transition-all bg-zinc-100 text-zinc-950 hover:bg-zinc-200 shadow-xl"
                  >
                    Get Started
                  </Link>
                )}
      ...
      {isCancelModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
          ...
          />
          <div className="relative w-full max-w-lg scale-up-center overflow-hidden rounded-[32px] border border-white/5 bg-slate-950 p-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200" style={{ WebkitMaskImage: "-webkit-radial-gradient(white, black)" }}>
            <div className="flex flex-col items-center text-center">
              ...
              <div className="mb-8 w-full space-y-4 rounded-3xl border border-white/5 bg-zinc-900/20 p-6 text-left font-sans">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-zinc-455">The Hobby plan only supports 3 active API keys. Please select your favorites.</p>
                </div>
