...
export function PlanHero({ plan, limit, usage, nextBillingDate, isUnlimited, billingInterval = "month" }: PlanHeroProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[32px] border border-zinc-200 bg-white p-8 shadow-sm">
      {/* Background Accent */}
      <div className="absolute -right-24 -top-24 h-64 w-64 rounded-full bg-emerald-500/5 blur-[80px]" />
      <div className="absolute -left-24 -bottom-24 h-64 w-64 rounded-full bg-zinc-900/5 blur-[80px]" />

      <div className="relative z-10 flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
        <div className="space-y-4">
          <div className="space-y-1">
            <p className="font-mono text-[10px] font-bold uppercase tracking-[0.3em] text-zinc-400">Current Subscription</p>
            <h2 className="font-serif text-4xl font-bold tracking-tight italic">{plan}</h2>
          </div>
          ...
          <div className="flex items-end justify-between">
            <div className="space-y-1">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-serif font-bold italic">{usage.toLocaleString()}</span>
                <span className="text-[10px] font-bold text-zinc-400">/ {isUnlimited ? '∞' : limit.toLocaleString()} credits</span>
              </div>
            </div>
            <div className="text-right">
              <span className="text-xs font-black text-zinc-900">{Math.round(pct)}%</span>
            </div>
          </div>
