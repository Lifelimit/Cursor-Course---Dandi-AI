...
function RepositorySourceEvidenceComponent({
  sources,
  sourceCount,
  topMatch,
  lowConfidence,
  ...
  const [openSourceKeys, setOpenSourceKeys] = useState<Set<string>>(() => new Set());
    ...
    <details className="group mx-auto mt-7 max-w-3xl border-t border-emerald-300/10 pt-4 xl:max-w-[78ch]">
      ...
      <div className="mt-3 space-y-1.5">
        {sources.map((src, sourceIndex) => {
          const sourceKey = `${src.chunkId || src.filePath}-${sourceIndex}`;
          const isSourceOpen = openSourceKeys.has(sourceKey);
...
export const RepositorySourceEvidence = memo(RepositorySourceEvidenceComponent);
