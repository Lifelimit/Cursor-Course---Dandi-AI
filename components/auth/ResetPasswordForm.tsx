  ...
  return (
    <div className="space-y-7">
      ...
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        <div className="space-y-2"><label htmlFor="new-password" className="dandi-label ml-1">New password</label><div className="relative"><input id="new-password" name="password" type={showPassword ? "text" : "password"} autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} required minLength={12} disabled={isLoading} aria-invalid={errorKind === "weak-password" ? "true" : undefined} className="dandi-field px-4 py-3.5 pr-14 text-sm font-medium sm:px-5 sm:pr-14" placeholder="••••••••" /><button type="button" onClick={() => setShowPassword((visible) => !visible)} disabled={isLoading} className="absolute inset-y-0 right-3 flex w-10 items-center justify-center rounded-lg text-slate-500 hover:text-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300/70" aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? "◉" : "◌"}</button></div><p className="px-1 text-xs leading-5 text-slate-500">Use at least 12 characters.</p></div>
