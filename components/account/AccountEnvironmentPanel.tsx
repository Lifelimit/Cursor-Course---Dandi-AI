...
export function AccountEnvironmentPanel() {
  ...
  return (
    <CommandPanel id="account-integrations-panel" role="tabpanel" aria-labelledby="integrations-tab" className="space-y-8 p-8 md:p-10">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="space-y-1">
          ...
          <p className="max-w-3xl text-sm text-slate-400">
            Connect a GitHub App installation so Dandi can see repository access granted on GitHub.
          </p>
        </div>
        <button
          type="button"
      ...
      {(connectedNotice || notice || callbackError || errorMessage) && (
        <div
        ...
        >
          <p className="text-xs font-semibold leading-5">
            {callbackError || errorMessage || notice || "GitHub connected successfully."}
          </p>
        </div>
      )}
          ...
          {isLoading ? (
            ...
            <div className="mt-6 space-y-5">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                >
                  {isRemoving ? "Removing..." : "Remove from Dandi"}
                </button>
              </div>
              <p className="text-[11px] font-medium leading-5 text-slate-500">
                Removing the connection deletes Dandi&apos;s local installation record only. Manage or uninstall the app on GitHub to revoke GitHub-side access.
              </p>
            </div>
          ) : (
            <div className="mt-6 space-y-5">
              <p className="text-xs font-semibold leading-5 text-slate-400">
                Connecting GitHub lets Dandi discover the repositories granted to the GitHub App installation. Repository contents are not stored by this connection step.
              </p>
        ...
        <section className="rounded-lg border border-white/10 bg-slate-950/40 p-6 shadow-xl backdrop-blur-xl">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-xs font-semibold text-slate-500">
                {isConnected
                  ? `${status?.repositories.length || 0} loaded from GitHub${installation?.repositoryCount ? ` of ${installation.repositoryCount}` : ""}.`
          ...
          <div className="mt-5 overflow-hidden rounded-lg border border-white/10 bg-slate-950/50">
            {isLoading ? (
              ...
              <div className="p-6 text-sm font-semibold leading-6 text-slate-500">
                {isConnected
                  ? "GitHub did not return repositories for this installation. Manage the installation on GitHub to grant repository access."
          ...
          {isConnected && status && status.repositories.length > visibleRepositories.length && (
            <p className="mt-3 text-[11px] font-medium text-slate-500">
              Showing the first {visibleRepositories.length} repositories. Manage the full repository selection on GitHub.
            </p>
          )}
        </section>
      </div>
