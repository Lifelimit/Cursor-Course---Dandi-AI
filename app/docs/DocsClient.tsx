...
const summaryRequest = `curl -X POST https://dandi.ai/api/github-summarizer \\
  -H "Content-Type: application/json" \\
  -H "x-api-key: YOUR_API_KEY" \\
  -d '{"githubUrl":"https://github.com/facebook/react"}'`;
const prepareRequest = `curl -X POST https://dandi.ai/api/rag/ingest \\
  -H "Content-Type: application/json" \\
  -H "x-api-key: YOUR_API_KEY" \\
  -d '{"githubUrl":"https://github.com/facebook/react"}'`;
const askRequest = `curl -N -X POST https://dandi.ai/api/rag/chat \\
  -H "Content-Type: application/json" \\
  -H "x-api-key: YOUR_API_KEY" \\
  -d '{
...
function NavList({ activeId, query, onNavigate }: { activeId: string; query: string; onNavigate?: () => void }) {
  ...
  return <div className="space-y-6">{groups.map((group) => <div key={group.label} className="space-y-2"><p className="dandi-type-metadata text-zinc-500">{group.label}</p><ul className="space-y-1">{group.items.map((item) => <li key={item.id}><a href={`#${item.id}`} onClick={onNavigate} aria-current={activeId === item.id ? "location" : undefined} className={`block rounded-lg border px-3 py-2 text-xs font-bold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300/60 ${activeId === item.id ? "border-emerald-300/25 bg-emerald-300/[0.08] text-emerald-200" : "border-transparent text-slate-400 hover:border-white/10 hover:bg-white/[0.03] hover:text-white"}`}>{item.label}</a></li>)}</ul></div>)}</div>;
}
...
export default function DocsClient({ initialSession }: { initialSession: Session | null }) {
  ...
  return <div className="min-h-screen bg-[#05070b] font-sans text-slate-200 selection:bg-emerald-500/20 selection:text-emerald-100">
    ...
    <a href="#docs-content" className="sr-only z-50 rounded bg-emerald-200 px-3 py-2 text-sm font-bold text-slate-950 focus:not-sr-only focus:fixed focus:left-4 focus:top-4">Skip to documentation</a>
    <main id="docs-content" className="mx-auto max-w-7xl px-4 pb-24 pt-28 sm:px-6 md:pt-36">
      <div className="grid gap-8 lg:grid-cols-[15rem_minmax(0,1fr)] lg:gap-12">
        <aside aria-label="Documentation navigation" className="hidden h-fit space-y-5 lg:sticky lg:top-28 lg:block">
          ...
          <NavList activeId={activeId} query={query} />
        ...
        <div className="min-w-0 space-y-16">
          ...
          <div className="lg:hidden"><button type="button" onClick={() => setMobileNavOpen((open) => !open)} aria-expanded={mobileNavOpen} aria-controls="mobile-docs-navigation" className="flex w-full items-center justify-between rounded-xl border border-white/10 bg-slate-950/70 px-4 py-3 text-xs font-bold text-slate-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300/60">Browse documentation <span aria-hidden="true">{mobileNavOpen ? "−" : "+"}</span></button>{mobileNavOpen && <div id="mobile-docs-navigation" className="mt-2 rounded-xl border border-white/10 bg-slate-950/70 p-4"><label className="block"><span className="sr-only">Search documentation</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search endpoints, errors, or concepts" className="mb-5 w-full rounded-lg border border-white/10 bg-black/20 px-3 py-2.5 text-xs text-slate-200 outline-none placeholder:text-slate-600 focus:border-emerald-300/45" /></label><NavList activeId={activeId} query={query} onNavigate={() => setMobileNavOpen(false)} /></div>}</div>

          <section id="overview" className="scroll-mt-28 space-y-6"><SectionHeader eyebrow="Product model" title="Repository intelligence flow">Summarize gives an immediate overview of a public repository. Prepare indexes repository content into retrieval-ready chunks. Ask retrieves relevant prepared context and returns an answer with source evidence when matches are available.</SectionHeader><div className="grid gap-3 sm:grid-cols-3">{[["01", "Summarize", "Inspect structure, purpose, and key components immediately."], ["02", "Prepare", "Create or restore a durable completed preparation job for retrieval."], ["03", "Ask", "Query the prepared repository and inspect associated evidence."]].map(([number, title, text]) => <CommandPanel key={title} className="space-y-2 p-4"><span className="dandi-type-metadata text-emerald-300">{number}</span><h3 className="font-bold text-slate-100">{title}</h3><p className="text-xs leading-5 text-slate-400">{text}</p></CommandPanel>)}</div><p className="max-w-3xl text-sm leading-7 text-slate-400"><strong className="font-semibold text-slate-200">Preparation enables Ask; it is not required for Summary.</strong> Summary does not automatically prepare a repository.</p></section>

          <section id="quickstart" className="scroll-mt-28 space-y-6"><SectionHeader eyebrow="First request" title="Quickstart">1. Create or select an API key in your dashboard. 2. Choose a public GitHub repository URL. 3. Send this Summary request. 4. Inspect the streamed response and metadata. 5. Open the same workflow in Playground.</SectionHeader><CodeExample title="summary-request" language="curl" value={summaryRequest} /><Link href="/playground?mode=summary" className="inline-flex rounded-lg border border-cyan-300/20 bg-cyan-300/[0.06] px-3 py-2 text-[10px] font-black uppercase tracking-widest text-cyan-100 transition hover:border-cyan-300/45 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200/70">Open Summarize in Playground →</Link></section>

          <section id="authentication" className="scroll-mt-28 space-y-6"><SectionHeader eyebrow="Authentication" title="Send your API key with every request">All documented endpoints accept JSON requests and require <code className="rounded border border-white/10 bg-slate-950 px-1.5 py-0.5 font-mono text-xs text-slate-200">x-api-key: YOUR_API_KEY</code>. Keep keys server-side; never place a live key in browser code or a public repository.</SectionHeader><ParameterTable label="Required request headers"><thead><tr><th>Header</th><th>Required</th><th>Value</th></tr></thead><tbody><tr><td><code className="text-cyan-200">Content-Type</code></td><td><strong className="text-rose-200">Yes</strong></td><td><code>application/json</code></td></tr><tr><td><code className="text-cyan-200">x-api-key</code></td><td><strong className="text-rose-200">Yes</strong></td><td>Your Dandi API key</td></tr></tbody></ParameterTable><div className="rounded-xl border border-amber-300/20 bg-amber-300/[0.05] p-4 text-sm leading-6 text-slate-400"><p className="dandi-type-metadata text-amber-200">Repository access</p><p className="mt-2">Public GitHub URLs can be requested directly. For a private repository, connect GitHub and grant Dandi access to that repository; a missing or insufficient installation is returned as a GitHub authorization error.</p></div></section>

          <section id="summarize" className="scroll-mt-28 space-y-6"><EndpointHeader title="Repository Summary" path="/api/github-summarizer" playgroundHref="/playground?mode=summary" playgroundLabel="Open Summarize in Playground">Get an immediate repository overview: structure, purpose, and key components. This endpoint does not prepare the repository for later questions.</EndpointHeader><div className="grid gap-6 xl:grid-cols-2"><div className="min-w-0 space-y-4"><h3 className="dandi-type-metadata text-slate-500">Request body</h3><ParameterTable label="Repository Summary request parameters"><thead><tr><th>Parameter</th><th>Required</th><th>Description</th></tr></thead><tbody><tr><td><code className="text-cyan-200">githubUrl</code></td><td><strong className="text-rose-200">Yes</strong></td><td>Full GitHub repository URL, such as <code>https://github.com/owner/repo</code>.</td></tr></tbody></ParameterTable></div><div className="min-w-0 space-y-3"><h3 className="dandi-type-metadata text-slate-500">Important behavior</h3><p className="text-sm leading-7 text-slate-400">The successful body is a streamed text summary. Repository metadata is supplied in the <code className="text-cyan-200">x-github-metadata</code> response header as base64-encoded JSON.</p><p className="text-sm leading-7 text-slate-400">Invalid URLs return 400; unavailable repositories can return 404, 403, or 422 depending on access and metadata retrieval.</p></div></div><CodeExample title="repository-summary-request" language="curl" value={summaryRequest} /></section>

          <section id="prepare" className="scroll-mt-28 space-y-6"><EndpointHeader title="Prepare repository" path="/api/rag/ingest" playgroundHref="/playground?mode=ask" playgroundLabel="Open Prepare & Ask in Playground">Index a repository into retrieval-ready chunks. Prepare is required before asking source-backed questions; completed jobs may be restored rather than indexed again.</EndpointHeader><ParameterTable label="Prepare repository request parameters"><thead><tr><th>Parameter</th><th>Required</th><th>Description</th></tr></thead><tbody><tr><td><code className="text-cyan-200">githubUrl</code></td><td><strong className="text-rose-200">Yes</strong></td><td>Full GitHub repository URL.</td></tr></tbody></ParameterTable><div className="grid gap-5 xl:grid-cols-2"><CodeExample title="prepare-repository-request" language="curl" value={prepareRequest} /><CodeExample title="prepare-repository-response" language="json" tone="emerald" value={prepareResponse} /></div><p className="text-sm leading-7 text-slate-400">Use <code className="text-cyan-200">GET /api/rag/ingest?jobId=JOB_ID</code> with the same API key to inspect a job. A completed job includes its status, step, and available preparation summary such as file and chunk counts when returned.</p></section>

          <section id="ask" className="scroll-mt-28 space-y-6"><EndpointHeader title="Ask a repository" path="/api/rag/chat" playgroundHref="/playground?mode=ask" playgroundLabel="Open Prepare & Ask in Playground">Retrieve relevant prepared repository context and generate a streamed answer. The response can attach source evidence for the matched repository content.</EndpointHeader><ParameterTable label="Ask a repository request parameters"><thead><tr><th>Parameter</th><th>Required</th><th>Description</th></tr></thead><tbody><tr><td><code className="text-cyan-200">githubUrl</code></td><td><strong className="text-rose-200">Yes</strong></td><td>The prepared GitHub repository URL.</td></tr><tr><td><code className="text-cyan-200">messages</code></td><td><strong className="text-rose-200">Yes</strong></td><td>Message objects with <code>role</code> (<code>user</code> or <code>assistant</code>) and <code>content</code> strings.</td></tr></tbody></ParameterTable><CodeExample title="ask-repository-request" language="curl" tone="violet" value={askRequest} /><div className="rounded-xl border border-violet-300/20 bg-violet-300/[0.05] p-4 text-sm leading-7 text-slate-400"><p className="dandi-type-metadata text-violet-200">Evidence, not hidden reasoning</p><p className="mt-2">Answers stream in the response body. The optional <code>x-rag-sources</code> header describes retrieved repository evidence; it is context associated with the prepared repository, not chain-of-thought or a promise of an exact source count.</p></div></section>
