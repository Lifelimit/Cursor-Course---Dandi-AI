...
function EmptyPaymentState({ hasNoBillingProfile, hasSecondaryMethods, onAdd }: { hasNoBillingProfile: boolean; hasSecondaryMethods: boolean; onAdd: () => void }) {
  const title = hasSecondaryMethods ? "No default payment method." : "No payment method yet.";
  const description = hasSecondaryMethods
    ? "Choose Set as default on a saved method, or add another card and make it your default."
    : hasNoBillingProfile
      ? "A billing profile is created securely through Stripe when you add a card or begin a paid plan."
      : "Add a payment method before upgrading so future renewals can complete without interruption.";

  return <div className="mt-5 rounded-2xl border border-dashed border-white/10 bg-black/10 p-6 text-center"><p className="text-sm font-bold text-white">{title}</p><p className="mx-auto mt-2 max-w-md text-xs leading-6 text-slate-400">{description}</p><button type="button" onClick={onAdd} className="mt-5 min-h-10 rounded-full border border-emerald-300/25 bg-emerald-300/10 px-4 text-[10px] font-black uppercase tracking-[0.14em] text-emerald-100 hover:bg-emerald-300/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300">Add payment method</button></div>;
}
