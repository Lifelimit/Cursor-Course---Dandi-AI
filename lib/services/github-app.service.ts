...
export async function verifyGitHubUserCanAccessInstallation(input: {
  userAccessToken: string;
  installationId: number;
}) {
  await githubJson<{ total_count?: number }>(
...
export async function persistGitHubAppInstallation(input: {
  ...
  const [installation, repoList] = await Promise.all([
    ...
    listGitHubInstallationRepositories(input.installationId, { maxPages: 1 }),
  ]);
  ...
  const row = {
    ...
    repository_count: repoList.totalCount,
    connected_at: now,
    last_sync_at: now,
    updated_at: now,
  };
...
export async function updateGitHubInstallationSyncMetadata(input: {
  db: SupabaseClient;
  userId: string;
  installationId: number;
  ...
  if (error) {
    throw new Error("Dandi could not update GitHub installation metadata.");
  }
}
