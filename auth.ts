      ...
      if (session.user) {
        ...
        try {
          const { data, error } = await supabaseAdmin
            .from("profiles")
            .select("plan, billing_interval, stripe_customer_id, billing_street, billing_city, billing_state, billing_zip, billing_country, full_name, payment_method_last4, payment_method_brand, payment_method_expiry")
          ...
          if (data && !error) {
            ...
            session.user.stripe_customer_id = data.stripe_customer_id;
